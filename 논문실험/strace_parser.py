...
def pid_dict_intialize(pid_):
    ...
    if not pid_ in pid_fd_read_offset_cnt:
        pid_fd_read_offset_cnt[pid_]={}
    ...
    if not pid_ in pid_fd_write_offset_cnt:
        pid_fd_write_offset_cnt[pid_]={}
...
def open_process(line):
    ...
    global pid_fd_read_offset, pid_fd_read_offset_len
    global pid_fd_write_offset, pid_fd_write_offset_len
    global seq_request_size, seq_request_len, pid_list
    ...
    pid_fd_read_offset_cnt[pid_][fd_]=0
    ...
    pid_fd_write_offset_cnt[pid_][fd_]=0
...
def read_process(line):
    ...
    global pid_fd_read_offset_cnt, seq_request_len
    comm=line[1]
    fd_=line[2]
    ...
    if(r_byte>0):
        ...
        try:
            if pid_fd_read_offset_before[pid_][fd_]==pid_fd_read_offset[pid_][fd_]:
                ...
                pid_fd_read_offset_cnt[pid_][fd_]+=1
            else:
                if pid_fd_read_offset_len[pid_][fd_]>=1:
                    #print(pid_fd_read_offset_len[pid_][fd_], line)
                    seq_request_size = np.append(seq_request_size, pid_fd_read_offset_len[pid_][fd_])
                    seq_request_len = np.append(seq_request_len, pid_fd_read_offset_cnt[pid_][fd_])
                pid_fd_read_offset_len[pid_][fd_]=r_byte
                pid_fd_read_offset_cnt[pid_][fd_]=1
...
def write_process(line):
    global request_size
    global pid_fd_write_offset, pid_fd_write_offset_before
    global pid_fd_write_offset_len, seq_request_size
    global pid_fd_write_offset_cnt, seq_request_len
    ...
    if(w_byte>0):
        ...
        if pid_fd_write_offset_before[pid_][fd_]==pid_fd_write_offset[pid_][fd_]:
            pid_fd_write_offset_len[pid_][fd_]+=1
            pid_fd_write_offset_cnt[pid_][fd_]+=1
        else:
            if pid_fd_write_offset_len[pid_][fd_]>=1:
                #print(pid_fd_write_offset_len[pid_][fd_], line)
                seq_request_size = np.append(seq_request_size, pid_fd_write_offset_len[pid_][fd_])
                seq_request_len = np.append(seq_request_len, pid_fd_write_offset_cnt[pid_][fd_])
...
def parser():
    ...
    print("open count: ", open_cnt)
    print("sync count: ", sync_cnt)
    #print(pid_file_fd)
    #print(pid_fd_file)
    print("pid count", len(pid_list))
    print("filenum",len(files))
    ...
    if np.size(seq_request_size):
        ...
        seq_over32=seq_request_size[np.where(seq_request_len>=32)]
        seq_under32=seq_request_size[np.where(seq_request_len<32)]
        print("seq, rand", np.size(seq_over32), np.size(seq_under32))
        print("seq vs rand:",np.sum(seq_over32),"/",np.sum(seq_under32))
