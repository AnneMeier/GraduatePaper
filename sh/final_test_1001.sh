ext4_format() {
	...
	case $3 in
		journal_async_commit)
			mount -t ext4 -o $4,journal_async_commit,data=$2 -w /dev/$1 /home/j/hd3;;
		*)
	...
	for datacow in datacow nodatacow;do
		for secsize in 1024 2048 4096;do
			for nodesize in 4096 8192 16384;do
				...
				#fs_test_cohe $2_btrfs/${datacow}_${secsize}_${nodesize_mul}
				mount_test
				my_umount $1
				echo umount complete
				...
				#fs_test_scat $2_btrfs/${datacow}_${secsize}_${nodesize_mul}
				mount_test
				my_umount $1
				echo umount complete
			...
			for rr in 0 25 50 75 100; do
				for rs in 4 32 128; do
					fio --directory=/home/j/hd3 --name fio_test_file --ioengine=${io_en} --rw=${rp} --rwmixread=${rr} --bs=${rs} \
						--size=1G --numjobs=1 --time_based --runtime=20 --group_reporting \
	...
	for io_en in sync libaio pvsync2; do
		for rp in rw randrw; do
			for rr in 0 25 50 75 100; do
				for rs in 4 32 128; do
					fio --directory=/home/j/hd3 --name fio_test_file --ioengine=${io_en} --rw=${rp} --rwmixread=${rr} --bs=${rs} \
						--size=1m --numjobs=100 --time_based --runtime=20 --group_reporting \
